  ...
  $('top3').innerHTML=current.top10.slice(0,3).map(c=>`
    <article class="creator-card ${c.handle===sel?'active':''}" data-handle="${escapeAttr(c.handle)}">
      ...
      <div class="creator-main">
        <div class="avatar">${initials(c.handle)}</div>
        <div class="creator-text">
          <div class="handle">${escapeHtml(c.handle)}</div>
          <div class="creator-name">${escapeHtml(safeName(c.name,c.handle))}</div>
          <div class="niche">${escapeHtml(c.country)} · ${escapeHtml(c.niche)} · ${escapeHtml(c.platform)}</div>
        </div>
...
function renderRanking(){
  ...
  document.querySelectorAll('.rank-row').forEach(el=>{
    el.addEventListener('click', async ()=>{
      ...
      // Deliberately do not auto-scroll: the probability arithmetic next to
      // the ranking should be visible immediately after a candidate click.
    });
  });
}
...
function renderSigmoid(logit, probability){
  ...
  const line=(x1,y1,x2,y2,stroke='#dfe4ea',width=1,dash='')=>{
  ...
  const text=(x,y,value,size=9,fill='#687183',anchor='middle',weight='400')=>{
  ...
  [0,.25,.5,.75,1].forEach(v=>{line(L,Y(v),W-R,Y(v),'#eef1f4');text(L-6,Y(v)+3,v.toFixed(v===0||v===1?0:2),8,'#7c8593','end');});
  [-6,-4,-2,0,2,4,6].forEach(v=>{line(X(v),T,X(v),H-B,'#f1f3f6');text(X(v),H-17,String(v),8);});
  line(L,T,L,H-B,'#9da7b5',1.1); line(L,H-B,W-R,H-B,'#9da7b5',1.1);
  ...
  const path=document.createElementNS(NS,'path');path.setAttribute('d',d);path.setAttribute('fill','none');path.setAttribute('stroke','#a91f32');path.setAttribute('stroke-width','3');svg.appendChild(path);

  const px=Math.max(xmin,Math.min(xmax,logit));
  line(X(px),Y(probability),X(px),H-B,'#9b3341',1,'4 3');
  line(L,Y(probability),X(px),Y(probability),'#d5a3aa',1,'3 3');
  const dot=document.createElementNS(NS,'circle');dot.setAttribute('cx',X(px));dot.setAttribute('cy',Y(probability));dot.setAttribute('r','5');dot.setAttribute('fill','#b52437');dot.setAttribute('stroke','#fff');dot.setAttribute('stroke-width','2');svg.appendChild(dot);
  text(X(px),Math.max(12,Y(probability)-11),`z=${Number(logit).toFixed(3)}`,9,'#8f1728','middle','700');
  text(W-12,13,`${(probability*100).toFixed(2)}%`,12,'#8f1728','end','800');
  text((L+W-R)/2,H-4,'Logit z',9,'#5f6878');
  const ylabel=text(10,(T+H-B)/2,'P(Y=1)',9,'#5f6878'); ylabel.setAttribute('transform',`rotate(-90 10 ${(T+H-B)/2})`);
...
function renderDecisionPath(){
  ...
  $('decisionPath').innerHTML=s.decision_path.map((p,i)=>`
    <div class="path-step"><span class="path-num">${i+1}</span><div><b>${escapeHtml(p.feature_label)}</b> ${p.operator} ${escapeHtml(p.threshold_label)}<br><span style="color:#6d7687">Actual: ${escapeHtml(p.actual_label)}</span></div></div>`).join('')+
...
function renderSelected(){
  ...
  $('selectedInfo').innerHTML=`
    <div class="selected-name">${escapeHtml(s.handle)} ${s.verified?'<span class="verified-chip">Verified</span>':''}</div>
...
function layoutTree(root){
  ...
  visit(root,0);
  const width=Math.max(760,leafCursor+20);
  const height=Math.max(260,yStart+maxDepth*yGap+55);
...
function renderTree(){
  ...
  const nodeHalfW=52,nodeHalfH=22;
  ...
  edges.forEach(e=>{
    ...
    line.setAttribute('stroke',selected?'#b52437':'#b2bac5'); line.setAttribute('stroke-width',selected?'3':'1.5'); svg.appendChild(line);
    const label=document.createElementNS(NS,'text');
    label.setAttribute('x',(a.x+b.x)/2); label.setAttribute('y',(a.y+b.y)/2-4); label.setAttribute('text-anchor','middle');
    label.setAttribute('font-size','9'); label.setAttribute('font-weight',selected?'700':'500'); label.setAttribute('fill',selected?'#b52437':'#7c8593'); label.textContent=e.branch; svg.appendChild(label);
  });

  nodes.forEach(n=>{
    const selected=pathSet.has(n.id); const g=document.createElementNS(NS,'g');
    const rect=document.createElementNS(NS,'rect');
    rect.setAttribute('x',n.x-nodeHalfW); rect.setAttribute('y',n.y-nodeHalfH); rect.setAttribute('width',nodeHalfW*2); rect.setAttribute('height',nodeHalfH*2); rect.setAttribute('rx','6');
    rect.setAttribute('fill',selected?'#fff0f2':'#f1f4f7'); rect.setAttribute('stroke',selected?'#b52437':'#a7b1be'); rect.setAttribute('stroke-width',selected?'2.3':'1.1'); g.appendChild(rect);
    const t=document.createElementNS(NS,'text');
    t.setAttribute('x',n.x); t.setAttribute('y',n.y-7); t.setAttribute('text-anchor','middle'); t.setAttribute('font-size','8.5');
    t.setAttribute('font-weight',selected?'700':'600'); t.setAttribute('fill',selected?'#8f1728':'#354154');
    ...
    lines.forEach((txt,i)=>{ const sp=document.createElementNS(NS,'tspan'); sp.setAttribute('x',n.x); sp.setAttribute('dy',i===0?'0':'13'); sp.textContent=txt; t.appendChild(sp); });
