...
def _profile_score(niche: str, profile: Dict[str, float]) -> float:
    ...
    if "fashion" in key:
        return float(profile.get("fashion", 0.65))
    return 0.08
...
def compute_brand_fit(niche: str, brand: str, campaign: str) -> float:
    ...
    # Brand and campaign are both relevant; campaign gets a slight edge because
    # the same brand can run different kinds of campaigns.
    return float(np.clip(0.45 * brand_score + 0.55 * campaign_score, 0.03, 1.0))
